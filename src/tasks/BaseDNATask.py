...
class BaseDNATask(BaseTask):
    ...
    def enable(self):
        ...
        if (
            ...
            logger.info(
                f"Starting {self.name}; stopping mutually exclusive "
                f"task {current_task.name}"
            )
            current_task.disable()
            current_task.unpause()
        self.onetime_seen = set()
        super().enable()

    def focus_game_window(self) -> None:
        """Restore the game, bring it forward, and give it keyboard focus."""
        hwnd_window = self.hwnd
        hwnd = getattr(hwnd_window, "hwnd", None)
        if not hwnd:
            raise RuntimeError("找不到目前已連接的遊戲視窗")
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                logger.debug(
                    f"Force foreground attempt {attempt} failed: {error}"
                )
            finally:
        ...
        if win32gui.GetForegroundWindow() != hwnd:
            raise RuntimeError("無法將遊戲視窗切換到 Windows 前景")
        self.log_info("已確認遊戲視窗位於 Windows 前景")
